...
class UsuarioWeb(SuperUser):
	...
	def setParameters(self, urlSite=None):
		...
		# proxie = self.getProxie().getProxies()
		# proxie = proxie[randrange(0, len(proxie))]
...
if __name__ == '__main__':
	#print('[X] -- Inicia Usuario Web')
	uw = UsuarioWeb('padreSonador', '192.191.192.2')
	#print('[X] -- Obtiene parametros de usuario')
	# uw.getUserParameters()
	#print('[X] -- Inicia parametros configuración')
	# uw.setParameters(urlSite='http://ecleneue.com/WaO')
	uw.closeSession()
	#print('[X] -- Inicia búsqueda')
	# uw.search('letshome', 'google')
	# uw.navigateToLink('http://ecleneue.com/TMj', 'adlfy','google')
